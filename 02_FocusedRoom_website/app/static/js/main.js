...
(function() {
  ...
  // Track Big Five Test Started
  const startTestButtons = document.querySelectorAll('#start-test-btn, .start-test-trigger');
  startTestButtons.forEach(btn => {
  ...
  if (subscribeForm) {
    subscribeForm.addEventListener('submit', () => {
      ...
      console.log('GA4: newsletter_signup');
    });
  }

  // Track Extension Download (most valuable!)
  const downloadButtons = document.querySelectorAll('a[href*="download"], a[href*="chrome.google.com"]');
  downloadButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      console.log('GA4: extension_download');
    });
  });

  // Track Blog Post Read (engagement)
  ...
  // Track Big Five Completion (high value!)
  const emailGateForm = document.getElementById('email-gate-form');
  if (emailGateForm) {
    emailGateForm.addEventListener('submit', () => {
      gtag('event', 'test_completed', {
        'event_category': 'conversion',
        'event_label': 'Big Five Test Completed',
        'value': 50
      });
      console.log('GA4: test_completed');
    });
  }
